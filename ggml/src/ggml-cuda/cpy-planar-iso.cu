 ...
 *
 * Two conversions: F16->planar3, F16->iso3
...
void ggml_cuda_init_planar_iso_constants() {
    ...
    // Generated from LCG PRNG with seed=42.
...
#ifdef __HIPCC__
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_planar_cos), h_cos, sizeof(h_cos));
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_planar_sin), h_sin, sizeof(h_sin));
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_iso_qw), h_qw, sizeof(h_qw));
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_iso_qx), h_qx, sizeof(h_qx));
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_iso_qy), h_qy, sizeof(h_qy));
    (void)hipMemcpyToSymbol(HIP_SYMBOL(d_iso_qz), h_qz, sizeof(h_qz));
#else
