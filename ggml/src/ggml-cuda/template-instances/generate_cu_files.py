...
TYPES_KV_PLANAR = ["GGML_TYPE_PLANAR3_0", "GGML_TYPE_ISO3_0", "GGML_TYPE_PLANAR4_0", "GGML_TYPE_ISO4_0"]
...
# Planar/ISO mixed pairs: K = planar/iso, V = standard
for type_k in TYPES_KV_PLANAR:
    for type_v in TYPES_KV:
        with open(f"fattn-vec-instance-{get_short_name(type_k)}-{get_short_name(type_v)}.cu", "w") as f:
            f.write(SOURCE_FATTN_VEC.format(type_k=type_k, type_v=type_v))
# Planar/ISO mixed pairs: K = f16, V = planar/iso
for type_v in TYPES_KV_PLANAR:
    with open(f"fattn-vec-instance-f16-{get_short_name(type_v)}.cu", "w") as f:
        f.write(SOURCE_FATTN_VEC.format(type_k="GGML_TYPE_F16", type_v=type_v))
